...
namespace Owlcat.Runtime.Visual.Effects.WeatherSystem
{
	public class WeatherLayerController : IWeatherEntityController, IDisposable
	{
		...
		private static readonly int _AlphaScale = Shader.PropertyToID("_AlphaScale");

		private bool m_isOnConsole;
		...
		public WeatherLayerController(Transform root, WeatherLayer layer)
		{
			...
			m_isOnConsole = Application.isConsolePlatform;
			m_EffectControllers = (from effect in m_Layer.Effects?.Where((WeatherEffect effect) => effect != null && effect.VisualEffectPrefab != null && !Disable && (!effect.DontUseOnConsole || !m_isOnConsole))
