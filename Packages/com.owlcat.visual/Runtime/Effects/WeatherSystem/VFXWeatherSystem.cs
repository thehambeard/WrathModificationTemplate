...
namespace Owlcat.Runtime.Visual.Effects.WeatherSystem
{
	public class VFXWeatherSystem : MonoBehaviour
	{
		...
		private IWeatherProfile m_ProfileBeforeOverride;

		private bool m_IsPhotoMode;
		...
		public void ForceSetWeatherProfile(IWeatherProfile profile, bool isPhotoMode = false, float intensity = 1f)
		{
			m_IsPhotoMode = isPhotoMode;
			Profile = profile;
			Reset();
			m_PreviousIntensity = (m_CurrentWeatherIntensity = (m_NewIntensity = intensity));
		}
		...
		public void Play()
		{
			...
			{
				m_ProfileBeforeOverride = Profile;
				Profile = m_ProfileForOverride;
			}
			if (UseDebugParams && DebugWeatherProfile != null)
			{
				Profile = DebugWeatherProfile;
			}
			...
			{
				if (ValidateLocationWeatherData())
				{
					...
					m_CustomEffectControllers = (from effect in Profile.CustomEffects?.Where((WeatherCustomEntitySettings setting) => setting != null && (!m_IsPhotoMode || !setting.IsSoundSetting))
		...
		private bool ValidateLocationWeatherData()
		{
			bool hasAreas = VFXTotalLocationWeatherData.HasAreas;
			if (Profile.Layers.SelectMany((WeatherLayer layer) => layer.Effects).Any((WeatherEffect effect) => effect.UseBakedLocationData) && !hasAreas)
			{
				return false;
			}
			return true;
		...
		{
			if (Profile == m_ProfileForOverride)
			{
				Profile = m_ProfileBeforeOverride;
			}
