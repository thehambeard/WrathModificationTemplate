...
namespace Owlcat.Runtime.Visual.Effects.LineRenderer
{
	...
	public class CompositeLineRenderer : ProceduralMesh
	{
		...
		protected override void AfterUpdateJobComplete()
		{
			...
			if (m_GenerateLightingData)
			{
				m_Mesh.SetVertexBufferData(m_VerticesLit, 0, 0, m_VertexCountActive, 0, flags);
			}
			else
			{
				m_Mesh.SetVertexBufferData(m_VerticesUnlit, 0, 0, m_VertexCountActive, 0, flags);
			}
			if (m_MeshIndicesIsDirty)
			{
				m_Mesh.SetIndexBufferParams(m_IndexCount, IndexFormat.UInt32);
				m_Mesh.SetIndexBufferData(m_Indices, 0, 0, m_IndexCountActive, flags);
				m_Mesh.SetSubMesh(0, new SubMeshDescriptor(0, m_IndexCountActive), flags);
