...
namespace Owlcat.Runtime.Visual.RenderPipeline.RendererFeatures.PositionBasedDynamics.Passes
{
	public class DebugPass : ScriptableRenderPass
	{
		...
		private Material m_ParticlesMaterial;

		private Material m_ConstraintsMaterial;
		...
		public override string Name => "PBD Debug Pass";

		public DebugPass(RenderPassEvent evt, Material particlesMaterial, Material constraintsMaterial, Material debugMaterial)
		{
			base.RenderPassEvent = evt;
			m_ParticlesMaterial = particlesMaterial;
			m_ConstraintsMaterial = constraintsMaterial;
		...
		public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
		{
			...
			CommandBuffer commandBuffer = CommandBufferPool.Get("PBD Debug Pass");
			using (new ProfilingScope(commandBuffer, m_ProfilingSampler))
			{
				...
				if (m_Feature.DebugSettings.ShowCollidersAabb)
				{
					commandBuffer.SetGlobalColor(PositionBasedDynamicsConstantBuffer._Color, Color.magenta);
					m_GpuData.CollidersSoA.SetGlobalData(commandBuffer);
					commandBuffer.DrawMeshInstancedIndirect(RenderingUtils.CubeMesh, 0, m_ParticlesMaterial, 2, m_Feature.DebugSoA.DrawCollidersAabbArgsBuffer);
				}
				if (m_Feature.DebugSettings.ShowForceVolumesAabb && m_GpuData.ForceVolumes != null && m_GpuData.ForceVolumes.Count > 0 && m_Feature.DebugSoA.DrawForceVolumeAabbArgsBuffer != null)
				{
					commandBuffer.SetGlobalColor(PositionBasedDynamicsConstantBuffer._Color, Color.magenta);
					m_GpuData.ForceVolumesSoA.SetGlobalData(commandBuffer);
					commandBuffer.DrawMeshInstancedIndirect(RenderingUtils.CubeMesh, 0, m_ParticlesMaterial, 3, m_Feature.DebugSoA.DrawForceVolumeAabbArgsBuffer);
				}
