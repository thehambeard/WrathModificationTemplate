	...
	public struct ExtractLightDataJob : IJobFor
	{
		...
		public void Execute(int index)
		{
			...
			if (lightDescriptor.VisibleLight.lightType == LightType.Spot)
			{
				...
				float num8 = Mathf.Cos((float)Math.PI / 180f * lightDescriptor.VisibleLight.spotAngle * 0.5f);
				float num9 = ((!(lightDescriptor.InnerSpotAngle > 0f)) ? math.cos(2f * math.atan(math.tan(lightDescriptor.VisibleLight.spotAngle * 0.5f * ((float)Math.PI / 180f)) * 46f / 64f) * 0.5f) : math.cos(lightDescriptor.InnerSpotAngle * ((float)Math.PI / 180f) * 0.5f));
