...
namespace Owlcat.Runtime.Visual.RenderPipeline
{
	public class ClusteredRenderer : ScriptableRenderer 
	{
		...
		private FogSettingsPass m_FogSettingsPass;
        ...
        private DrawGizmoPass m_DrawGizmoPreImagePass;
        private DrawGizmoPass m_DrawGizmoPostImagePass;
			...
			: base(data)
		{
			...
			m_FogSettingsPass = new FogSettingsPass(RenderPassEvent.AfterRenderingPrePasses);
            ...
            this.m_DrawGizmoPreImagePass = new DrawGizmoPass(GizmoSubset.PreImageEffects);
            this.m_DrawGizmoPostImagePass = new DrawGizmoPass(GizmoSubset.PostImageEffects);
        }
		...
		public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
		{
			...
			m_FogSettingsPass.Setup();
			EnqueuePass(m_FogSettingsPass);
...
#if UNITY_EDITOR
			if (Handles.ShouldRenderGizmos() && !EditorApplication.isPlaying)
			{
				EnqueuePass(m_DrawGizmoPreImagePass);
				EnqueuePass(m_DrawGizmoPostImagePass);
			}
#endif
			if (isLightingEnabled)
			{
		...
		{
			CommandBuffer commandBuffer = CommandBufferPool.Get("Setup Global Shader Keywords");
		...
		private void ApplyDebugSettings(ScriptableRenderContext context, ref RenderingData renderingData)
		{
			...
			CommandBuffer commandBuffer = CommandBufferPool.Get("Apply Display Settings");
