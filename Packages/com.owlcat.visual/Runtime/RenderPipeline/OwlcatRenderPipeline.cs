...
namespace Owlcat.Runtime.Visual.RenderPipeline
{
	public class OwlcatRenderPipeline : UnityEngine.Rendering.RenderPipeline
	{
		...
		protected override void Dispose(bool disposing)
		{
			...
			if (Asset.ScriptableRenderer != null)
			{
		...
		private void UpdateRealtimeGI()
		{
			...
			if (activeScene != m_ActiveScene)
			{
				...
				Light[] lights = Light.GetLights(UnityEngine.LightType.Point, 0);
		...
		public static void RenderSingleCamera(ScriptableRenderContext context, CameraChainDescriptor cameraDesc)
		{
			...
			CommandBuffer commandBuffer = CommandBufferPool.Get(cameraDesc.Camera.name);
		...
		private static void InitializeShadowData(out ShadowingData shadowData)
		{
			...
			float num = 1f;
			shadowData.AtlasSize = (int)((float)Asset.ShadowSettings.AtlasSize * num);
			shadowData.SpotLightResolution = (int)((float)Asset.ShadowSettings.SpotLightResolution * num);
			shadowData.DirectionalLightCascades = Asset.ShadowSettings.DirectionalLightCascades;
			shadowData.DirectionalLightCascadeResolution = (int)((float)Asset.ShadowSettings.DirectionalLightCascadeResolution * num);
			shadowData.PointLightResolution = (int)((float)Asset.ShadowSettings.PointLightResolution * num);
		...
		private static RenderTextureDescriptor CreateRenderTextureDescriptor(Camera camera, float renderScale, bool isStereoEnabled, bool isHdrEnabled, int msaaSamples = 1)
		{
			...
			RenderTextureFormat renderTextureFormat2 = ((RenderingUtils.SupportsRenderTextureFormat(RenderTextureFormat.RGB111110Float) & Application.isMobilePlatform) ? RenderTextureFormat.RGB111110Float : RenderTextureFormat.DefaultHDR);
