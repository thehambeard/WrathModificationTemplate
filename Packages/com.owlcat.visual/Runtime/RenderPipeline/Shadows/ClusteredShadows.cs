...
namespace Owlcat.Runtime.Visual.RenderPipeline.Shadows
{
	public class ClusteredShadows
	{
		...
		private void ProcessLight(VisibleLight light, int lightIndex, ref RenderingData renderingData)
		{
			...
			else
			{
				...
				case LightType.Spot:
				{
					...
					float frustumSize = Mathf.Tan(light.spotAngle * 0.5f * ((float)Math.PI / 180f)) * light.range;
		...
		{
			float num = angleInDeg * 0.5f * ((float)Math.PI / 180f);
		...
		private Matrix4x4 CreatePerspective(Vector2 fov, float n, float f)
		{
			...
			proj.m00 = 1f / Mathf.Tan((float)Math.PI / 180f * fov.x * 0.5f);
			proj.m11 = 1f / Mathf.Tan((float)Math.PI / 180f * fov.y * 0.5f);
