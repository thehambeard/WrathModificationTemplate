...
namespace Owlcat.Runtime.Visual.RenderPipeline.Passes
{
	public class DepthPyramidPass : ScriptableRenderPass
	{
		...
		public override string Name => "Build Depth Pyramid";
		...
		{
			CommandBuffer commandBuffer = CommandBufferPool.Get("Build Depth Pyramid");
			using (new ProfilingScope(commandBuffer, m_ProfilingSampler))
			{
				for (int i = 1; i < m_GBuffer.DepthPyramidLodCount; i++)
				{
					int4 @int = (int4)(float4)m_GBuffer.DepthPyramidMipRects[i];
					int4 int2 = (int4)(float4)m_GBuffer.DepthPyramidMipRects[i - 1];
