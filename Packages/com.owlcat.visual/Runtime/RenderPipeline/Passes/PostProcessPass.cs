...
namespace Owlcat.Runtime.Visual.RenderPipeline.Passes
{
	internal class PostProcessPass : ScriptableRenderPass
	{
		...
		private static class ShaderConstants
		{
			...
			public static readonly int _Sepia_Params = Shader.PropertyToID("_Sepia_Params");
		...
		private static readonly int _RadialBlurCenter = Shader.PropertyToID("_RadialBlurCenter");

		private static readonly int _RadialBlurStrength = Shader.PropertyToID("_RadialBlurStrength");

		private static readonly int _RadialBlurWidth = Shader.PropertyToID("_RadialBlurWidth");
		...
		public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
		{
			...
			m_Sepia = stack.GetComponent<Sepia>();
		...
		private void DoDaltonization(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination)
		{
			...
			cmd.SetGlobalVector(ShaderConstants._Params1, new Vector4((m_Daltonization.BrightnessFactor.value > 1E-05f) ? m_Daltonization.BrightnessFactor.value : 0.5f, ((m_Daltonization.ContrastFactor.value > 1E-05f) ? m_Daltonization.ContrastFactor.value : 0.5f) + 0.5f));
		...
		private void SetupSepia(Material material)
		{
			float value = m_Sepia.Intensity.value;
			material.SetVector(value: new Vector4(value, 0f, 0f, 0f), nameID: ShaderConstants._Sepia_Params);
		}
